...
function fnvHashView(hash, view) {
  var int = new BigInt(hash);

  for (var i = 0; i < view.byteLength; i++) {
    int = int.xor(view.getUint8(i)).multiply(FNV_PRIME).and(0xFFFFFFFF);
  }
...
WorldRenderer.prototype._loadMetadata = function () {
  ...
  var array = new Uint8Array(8), view = new DataView(array.buffer);

  new BigInt(this.world.seed).add(BACKGROUND_VARIANT_SEED).toBytes(array);
  this._bgVariantHashBase = fnvHashView(FNV_OFFSET_BASIS, view);

  new BigInt(this.world.seed).add(FOREGROUND_VARIANT_SEED).toBytes(array);
